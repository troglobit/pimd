...
print "Starting pimd ..."
nsenter --net="$R1" -- ../src/pimd -i R1 -f "/tmp/$NM/conf" -n -p "/tmp/$NM/r1.pid" -l debug -d all -u "/tmp/$NM/r1.sock" &
echo $! >> "/tmp/$NM/PIDs"
nsenter --net="$R2" -- ../src/pimd -i R2 -f "/tmp/$NM/conf" -n -p "/tmp/$NM/r2.pid" -l debug -d all -u "/tmp/$NM/r2.sock" &
echo $! >> "/tmp/$NM/PIDs"
sleep 1
