...
The PIM-SSM and IGMPv3 release, with major change to the =pimd.conf=
syntax!  Most, if not all, of the new features in this release are
courtesy of Markus Veranen <mailto:markus.veranen@gmail.com>!

** Changes and New Features
  - Support for PIM-SSM and IGMPv3.  By Markus Veranen
  - IGMPv3 is now default, use =phyint ifname igmpv2= for old behavior
  ...
  - Default IGMP querier timeout has changed from 255 sec to 42 sec
    ...
    - Built in IGMP /robustness value/ changed from 2 to 3
...
Major change in the =pimd.conf= syntax, still backwards compatible with
the old syntax.  Most of the change is to use dashes '-' instead of the
old underscore '_' as word separators:
...
=distance=.  See the README or the man page for more information on the
metric preference and admin distance confusion.  Again, the parser is
backwards compatible with the old syntax.
...
using the =configure= script.  For details, see =./configure --help=
  ...
  - Support for adding a "generationd ID" field to PIM Hello messages
  - Allow /32 for VIFs,
