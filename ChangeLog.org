...
*Note:* command line arguments have changed in a non-backwards fashion.
...
- Modify Join/Prune send logic to allow more than ~75 mcast receivers,
  issue #62, by Joonas Ruohonen <mailto:joonas.ruohonen@insta.fi>
- Stream optimization, issue #67, by Mika Joutsenvirta:
  ...
  - Create MRT entry once PIM Register is received even if no joins
    exists, saves 0-60 sec
- Allow use of loopback interface as long as MULTICAST flag is set,
  issue #89 by Vincent Bernat
- Added systemd unit file, issue #41
- Support for =disable-vifs= in =pimd.conf=, issue #56
